...
logo = (f''' 
  ...
  ⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
  ⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
                   {light_blue}「{light_blue}PAID TOOL{light_blue}」{light_blue} \n''')
...
def overview():
    print(f"\033[0;34m  ━━━━━━━━━━━━━━━━━[{light_blue}𝙔𝙊𝙐𝙍 𝘼𝘾𝘾𝙊𝙐𝙉𝙏𝙎{light_blue}]━━━━━━━━━━━━━━━━━━━")
    total_accounts = count_lines("/sdcard/Aqua/toka.txt")
    total_pages = count_lines("/sdcard/Aqua/tokp.txt")
    print(f"  {light_blue}     TOTAL ACCOUNTS: {dark_gray}{total_accounts}{light_blue}")
    print(f"  {light_blue}     TOTAL PAGES: {dark_gray}{total_pages}")
    print(f'{light_blue}  ══════════════════════════════════════════════════{wh}')
...
def greet_user():
    ...
    if current_time.hour < 12:
        print(f"    Good morning Vern, {light_blue}{user_name}!{dark_gray}")
    elif current_time.hour < 18:
        print(f"    Good afternoon Vern, {light_blue}{user_name}!{dark_gray}")
    else:
        print(f"    Good evening Vern, {light_blue}{user_name}!{dark_gray}")
...
def ip():
    ...
    print(f" {light_blue}  「{light_blue}State{light_blue}」{w}:{dark_gray}THIS IS PAID NOT FOR FREE")
    print(f" {light_blue}  「{light_blue}IP{light_blue}」{w}:{dark_gray}{ip_response.get('query', ' ')}")
    print(f"{light_blue}   「{light_blue}country{light_blue}」{w}:{dark_gray}{ip_response.get('country', ' ')}")
    print(f"{light_blue}   「{light_blue}ISP{r}」{w}:{dark_gray}{ip_response.get('isp', ' ')}")
    print(f"{light_blue}    「{light_blue}s-key{r}」{w}:{dark_gray}NO NEED FOR SKEY{light_blue}")
    linex()
...
def menu():
    ...
    clr()
    left_options = [
        ("1", f"{light_blue}START"), 
        ("2", f"{light_blue}REACT"), 
        ("0", f"{light_blue}EXIT")
    ]
    right_options = [
        ("3", f"{light_blue}COMMENT"),
        ("11", f"{light_blue}RESET")
    ]
    for (left_option, left_desc), (right_option, right_desc) in zip(left_options, right_options):
        print(f"{r}   「{r}{left_option}{r}」{r} {left_desc:<14}     {r}  「{r}{right_option}{r}」{r} {right_desc}")
    linex()
    choice = input(f'   {purple}「{light_blue}Vern Choose {purple}」{purple}»  ')
    menu_actions = {
        '1': Initialize,
        '2': QuickReact,
        '5': C_page,
        '4': lambda: print(f"{red} Before following get token first .{r}"),
        '3': comment_on_facebook_post,
        '6': AutomaticSharing,
        '11': lambda: reset(),
...
def Manual():
    ...
    #linex()
    user = input(f"{red}USER ID/EMAIL:{wh} ")
    #linex()
    passw = input(f"{red}PASSWORD:{wh} ")
    linex()
